...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id,student_id,play_code,status,attempt_type,eligibility_source,payment_method,amount_collected,operator_name,created_at,redeemed_at,completed_at')
    ...
    return json(res, 200, {
      ...
      totals: {
        ...
        cash_collected: total((r) => r.payment_method === 'cash'),
        digital_collected: total((r) => r.payment_method === 'digital'),
        free_trials: count((r) => r.attempt_type === 'trial'),
        run_signup_attempts: count((r) => r.eligibility_source === 'run_signup'),
