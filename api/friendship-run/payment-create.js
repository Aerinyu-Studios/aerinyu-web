...
async function uploadEvidence(supabase, dataUrl, normalized, kind) {
  ...
  const upload = await supabase.storage.from('friendship-run-payment-proofs').upload(path, buffer, {
...
export default async function handler(req, res) {
  ...
  let operator;
  try { operator = boothAccess(req); }
  catch { return json(res, 401, { error: 'Your booth session expired.' }); }
  ...
  try {
    ...
    if (attemptType === 'trial') {
      source = 'free_trial';
      method = 'none';
      const existingTrial = await supabase
        .from('friendship_run_payments')
        .select('id')
        .eq('student_id_normalized', normalized)
        .eq('attempt_type', 'trial')
        .limit(1);
      if (existingTrial.error) throw existingTrial.error;
      if (existingTrial.data?.length) {
        return json(res, 409, { error: 'A free trial has already been issued for this student ID.' });
      }
    } else {
      if (!['booth_payment', 'run_signup'].includes(source)) {
        return json(res, 400, { error: 'Choose how the player qualified for the official attempt.' });
      }

      if (source === 'run_signup') {
        method = 'run_signup';
        amount = 0;
        evidenceKind = 'run_registration_receipt';
        const existingRunWaiver = await supabase
          .from('friendship_run_payments')
          .select('id')
          .eq('student_id_normalized', normalized)
          .eq('eligibility_source', 'run_signup')
          .limit(1);
        if (existingRunWaiver.error) throw existingRunWaiver.error;
        if (existingRunWaiver.data?.length) {
          return json(res, 409, { error: 'This student ID has already used the Friendship Run signup entitlement.' });
        }
      } else {
        amount = 3;
        if (!['cash', 'digital'].includes(method)) {
          return json(res, 400, { error: 'Choose cash or digital payment.' });
        }
        evidenceKind = method === 'cash' ? 'cash_handover_photo' : 'digital_payment_receipt';
      }

      try {
        proofPath = await uploadEvidence(supabase, req.body?.proof_data, normalized, evidenceKind);
      } catch (error) {
        if (error.message === 'EVIDENCE_REQUIRED') {
          return json(res, 400, { error: source === 'run_signup' ? 'Take a photo of the Friendship Run registration receipt.' : method === 'cash' ? 'Take a cash handover photo first.' : 'Take a payment receipt photo first.' });
        }
    ...
    return json(res, 200, {
      payment: {
        ...
        play_code: record.play_code,
        attempt_type: record.attempt_type,
        eligibility_source: record.eligibility_source,
        payment_method: record.payment_method,
        amount_collected: Number(record.amount_collected || 0),
        expires_at: record.expires_at,
        operator_name: record.operator_name
      }
    });
  } catch (error) {
    console.error('Friendship Run code creation error:', error);
