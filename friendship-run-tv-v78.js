...
const FRIENDSHIP_RUN_TV_BUILD = 'v78-status-toggle';
...
function applyTechnicalStatusVisibility(){
  const show = displayConfig.settings?.show_live_sync_status === true;
  if(liveTransport){
    liveTransport.hidden = !show;
    liveTransport.setAttribute('aria-hidden', String(!show));
  }
  document.body.classList.toggle('show-live-sync-status', show);
}
...
async function loadDisplayConfig(){
  try{
    ...
    applyTechnicalStatusVisibility();
